...
HRESULT DXUTCreate3DEnvironment10( ID3D10Device* pd3dDeviceFromApp )
{
    ...
    pDXGIFactory->MakeWindowAssociation( DXUTGetHWNDFocus(), DXGI_MWA_NO_WINDOW_CHANGES );
...
HRESULT DXUTReset3DEnvironment10()
{
    ...
    // Call the app's swap chain lost callback
    ...
    if( SCDesc.Windowed != pDeviceSettings->d3d10.sd.Windowed )
    {
        ...
        {
            // If going from fullscreen to windowed, change mode then call
            // ResizeTarget so that we don't set mode twice.
            pSwapChain->ResizeBuffers( pDeviceSettings->d3d10.sd.BufferCount,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Width,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Height,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Format,
                                       0 );
        ...
        {
            // If going from windowed to fullscreen, call ResizeTarget then
            // change mode so that we don't set mode twice.
            pSwapChain->ResizeBuffers( pDeviceSettings->d3d10.sd.BufferCount,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Width,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Height,
                                       pDeviceSettings->d3d10.sd.BufferDesc.Format,
            ...
            IDXGIOutput *pOutput = GetDXUTState().GetD3D10OutputArray()[pDeviceSettings->d3d10.Output];
    ...
    // Setup the render target view and viewport
    hr = DXUTSetupD3D10Views( pd3dDevice, pDeviceSettings );
    if( FAILED(hr) )
    {
        DXUT_ERR( L"DXUTSetupD3D10Views", hr );
...
void DXUTCheckForWindowSizeChange()
{
    ...
    if( GetDXUTState().GetIgnoreSizeChange() || !GetDXUTState().GetDeviceCreated() || !DXUTIsWindowed() )
        return;
